...
let v150 =
	"FROM SIT\nexperienced team of Mobile Telecom Engineers makes the system a step ahead of competit";

let gid = randomGen();

const SYSTEM_ID = "sumeet";
const PASSWORD = "sumeet123";

let mobiles = ["+919082963535", "+918800904670", "+917022228859"];
	...
	return new Promise((resolve, reject) => {
		...
		session.bind_transceiver(
			...
			(pdu) => {
				...
				//logger.info(`after bind_trans pdu : ${JSON.stringify(pdu)}`);
...
async function main() {
	try {
		...
		setTimeout(() => {
			...
			let msg = getMessage("udh");
			let msg2 = getMessage("2udh");
			console.log(msg);
			console.log(msg2);
...
function getMessage(name) {
	...
	if (name.indexOf("ensingle") === 0) {
		pduObj = {
			...
			dest_addr_ton: 5,
			dest_addr_npi: 1,
			destination_addr: "+919768275634",
			short_message: `test time : polsumeet@gmail.com $5000`,
			esm_class: 0,
		...
		pduObj = {
			...
			sm_default_msg_id: 0,
			short_message: {
				udh: Buffer.from([5, 0, 3, gid, 2, 1]),
				message: v150,
			},
			more_messages_to_send: 1,
		};
		return pduObj;
	} else if (name.indexOf("2udh") === 0) {
		pduObj = {
			service_type: "",
			source_addr_ton: 5,
			source_addr_npi: 0,
...
function submitMulti() {
	...
	//logger.info("submitmulit");
	pool[0].submit_multi(pduObj, (resp) => {
		//logger.info(resp);
	});
}

function randomGen() {
	return Math.floor(Math.random() * 230); // returns a random integer from 0 to 9
}
